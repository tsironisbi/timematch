...
def train_timematch(
    ...
    if config.estimate_shift:
        shift_estimator = (
            "IS" if config.shift_estimator == "AM" else config.shift_estimator
        )
        ...
        pseudo_softmaxes = get_pseudo_labels(
            teacher, target_loader_no_aug, device, target_to_source_shift, n=None
        )
    ...
    for epoch in range(config.epochs):
        progress_bar = tqdm(
            range(steps_per_epoch), desc=f"TimeMatch Epoch {epoch + 1}/{config.epochs}"
        )
        loss_meter = AverageMeter()

        if config.estimate_shift:
            estimated_class_distr = estimate_class_distribution(
                all_pseudo_labels, config.num_classes
            )
            writer.add_scalar(
                "train/kl_d",
                kl_divergence(actual_class_distr, estimated_class_distr),
                epoch,
            )
            target_to_source_shift = estimate_temporal_shift(
                teacher,
            ...
            if epoch == 0:
                if config.shift_source:
                    ...
                    source_to_target_shift = 0
                min_shift, max_shift = min(target_to_source_shift, 0), max(
                    0, target_to_source_shift
                )
        ...
        for step in progress_bar:
            sample_source, (sample_target_weak, sample_target_strong) = next(
                source_iter
            ), next(target_iter)

            # Get pseudo labels from teacher
            pixels_t_weak, mask_t_weak, position_t_weak, extra_t_weak = to_cuda(
                sample_target_weak, device
            )
            with torch.no_grad():
            ...
            pixels_t, mask_t, position_t, extra_t = to_cuda(
                sample_target_strong, device
            )
            logits_target = None
            loss_target = 0.0
            if config.domain_specific_bn:
                logits_source = student.forward(
                    pixels_s, mask_s, position_s + source_to_target_shift, extra_s
                )
                if (
                    len(torch.nonzero(pseudo_mask)) >= 2
                ):  # at least 2 examples required for BN
            ...
            if step % config.log_step == 0:
                ...
                writer.add_scalar(
                    "train/target_updates", len(torch.nonzero(pseudo_mask)), global_step
                )
...
def update_ema_variables(model, ema, decay=0.99):
    for ema_v, model_v in zip(ema.state_dict().values(), model.state_dict().values()):
        ema_v.copy_(decay * ema_v + (1.0 - decay) * model_v)
...
def get_data_loaders(splits, config, balance_source=True):
    ...
    print(
        f"size of target dataset: {len(target_dataset)} ({len(target_loader_weak_strong)} batches)"
    )
...
def estimate_temporal_shift(
    ...
    for _ in tqdm(
        range(sample_size), desc=f"Estimating shift between [{min_shift}, {max_shift}]"
    ):
        ...
        shift_logits = torch.stack(
            [
                model.decoder(model.temporal_encoder(spatial_feats, positions + shift))
                for shift in shifts
            ],
            dim=1,
        )
    ...
    shift_softmaxes = (
        torch.cat(shift_softmaxes).cpu().numpy()
    )  # (N, n_shifts, n_classes)
    ...
    print(
        f"Most accurate shift {shifts[np.argmax(shift_acc_scores)]} with {np.max(shift_acc_scores):.3f}"
    )
    ...
    if shift_estimator == "IS":
        inception_score = np.mean(
            np.sum(
                p_yx * (np.log(p_yx + 1e-5) - np.log(p_y[np.newaxis] + 1e-5)), axis=2
            ),
            axis=0,
        )  # (n_shifts)
        ...
        print(
            f"Best Inception Score shift {best_shift} with accuracy {shift_acc_scores[best_shift_idx]:.3f}"
        )
        return best_shift
        ...
        best_shift = shifts[best_shift_idx]
        print(
            f"Best Entropy Score shift {best_shift} with accuracy {shift_acc_scores[best_shift_idx]:.3f}"
        )
        return best_shift

    elif shift_estimator == "AM":
        assert (
            class_distribution is not None
        ...
        kl_d = np.sum(
            c_train * (np.log(c_train + 1e-5) - np.log(one_hot_p_y + 1e-5)), axis=1
        )
