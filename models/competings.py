...
from .tae import get_positional_encoding
import copy
...
class GRU(nn.Module):
    ...
    def __init__(
        self, in_channels=128, hidden_dim=128, max_position=365, max_temporal_shift=100
    ):
        super(GRU, self).__init__()
        self.name = "GRU_h{}".format(hidden_dim)
        self.gru_cell = nn.GRU(
            input_size=in_channels, hidden_size=hidden_dim, batch_first=True
        )

        self.max_temporal_shift = max_temporal_shift
        self.position_enc = nn.Embedding.from_pretrained(
            get_positional_encoding(
                max_position + 2 * max_temporal_shift, self.in_channels, T=1000
            ),
            freeze=True,
        )
...
class TempConv(nn.Module):
    ...
    def __init__(
        self, input_size, nker, seq_len, nfc, max_position=365, max_temporal_shift=100
    ):
