...
def main(config):
    ...
    indices = {
        ...
        config.target: len(
            PixelSetData(config.data_root, config.target, source_classes)
        ),
    }
    ...
    for fold_num, splits in enumerate(folds):
        ...
        if not config.eval:
            ...
            if config.method == "timematch":
                ...
                train_supervised(
                    model, config, writer, splits, val_loader, device, best_model_path
                )
...
def get_dataset_size(data_root, dataset):
    ...
    return len(
        [
            name
            for name in os.listdir(os.path.join(dir, "data"))
...
def train_supervised(
    ...
    print(
        f"training dataset: {dataset_name}, n={len(dataset)}, batches={len(data_loader)}"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--source",
        default="denmark/32VNH/2017",
        help="source dataset",
        choices=available_tiles,
    )
    parser.add_argument(
        "--target",
        default="france/30TXT/2017",
        help="target dataset",
        choices=available_tiles,
    )
    parser.add_argument(
        ...
        default=3,
        type=int,
        help="Number of train/test folds for cross validation",
    )
    parser.add_argument(
        "--val_ratio",
        default=0.1,
    ...
    parser.add_argument(
        "--model", default="pseltae", choices=["psetae", "pseltae", "psetcnn", "psegru"]
    )
    parser.add_argument(
    ...
    dann.add_argument(
        "--adv_loss", type=str, default="DANN", choices=["DANN", "CDAN", "CDAN+E"]
    )
    dann.add_argument(
        "--use_default_optim",
        type=bool_flag,
        default=True,
        help="whether to use default optimizer",
    )
    dann.add_argument(
        "--weights", type=str, help="path to source trained model weights"
    )
    dann.add_argument(
        "--steps_per_epoch", type=int, default=500, help="n steps per epoch"
    )
    dann.add_argument(
        "--epochs", default=20, type=int, help="Number of epochs per fold"
    )
    dann.add_argument(
        "--trade_off", default=1.0, type=float, help="weight of adversarial loss"
    )
    ...
    jumbot.add_argument(
        "--eta1", default=0.01, type=float, help="feature comparison coefficient"
    )
    jumbot.add_argument(
        "--eta2", default=1.0, type=float, help="label comparison coefficient"
    )
    jumbot.add_argument(
        "--epsilon", default=0.01, type=float, help="marginal coefficient"
    )
    jumbot.add_argument(
        "--tau", default=0.5, type=float, help="entropic regularization"
    )
    ...
    timematch.add_argument(
        "--ema_decay", default=0.9999, type=float, help="decay rate for mean teacher"
    )
    timematch.add_argument(
        "--trade_off", type=float, default=2.0, help="weight for unsupervised loss"
    )
    timematch.add_argument(
        "--estimate_shift",
        type=bool_flag,
    ...
    timematch.add_argument(
        ...
        type=bool_flag,
        default=True,
        help="class balanced batches for source",
    )
    timematch.add_argument(
        "--use_focal_loss",
        type=bool_flag,
        default=True,
        help="use focal loss or cross entropy",
    )
    timematch.add_argument(
        "--shift_source",
        type=bool_flag,
    ...
    timematch.add_argument(
        ...
        type=int,
        default=60,
        help="maximum temporal shift to consider",
    )
    timematch.add_argument(
        "--domain_specific_bn",
        type=bool_flag,
    ...
    timematch.add_argument(
        "--shift_estimator", type=str, default="AM", choices=["AM", "IS", "ACC", "ENT"]
    )
    timematch.add_argument(
        "--run_validation",
        default=True,
    ...
    timematch.add_argument(
        ...
        type=bool_flag,
        default=True,
        help="output student or teacher",
    )
    ...
    if cfg.experiment_name is not None:
        cfg.tensorboard_log_dir = os.path.join(
            cfg.tensorboard_log_dir, cfg.experiment_name
        )
