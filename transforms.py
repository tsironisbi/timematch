...
class RandomSamplePixels(object):
    ...
    def __call__(self, sample):
        ...
        if S > self.num_pixels:
            ...
            x[..., :S] = pixels
            x[..., S:] = np.stack(
                [x[:, :, 0] for _ in range(S, self.num_pixels)], axis=-1
            )
            valid_pixels = np.array(
                [1 for _ in range(S)] + [0 for _ in range(S, self.num_pixels)]
            )
        else:
            x = pixels
            valid_pixels = np.ones(self.num_pixels)
        # Repeat valid_pixels across time
        valid_pixels = np.repeat(
            valid_pixels[np.newaxis].astype(np.float32), x.shape[0], axis=0
        )
        sample["pixels"] = x
...
class RandomSampleTimeSteps(object):
    ...
    def __call__(self, sample):
        if self.seq_length == -1:
            return sample
        pixels, date_positions, valid_pixels = (
            sample["pixels"],
            sample["positions"],
            sample["valid_pixels"],
        )
...
class Normalize(object):
    ...
    def __init__(self, max_pixel_value=65535):
        ...
        max_cover_ratio = 1.0
        self.max_extra_values = np.array(
            [max_perimeter, max_area, max_perimeter_area_ratio, max_cover_ratio]
        )

    def __call__(self, sample):
        sample["pixels"] = (
            np.clip(sample["pixels"], 0, self.max_pixel_value).astype(np.float32)
